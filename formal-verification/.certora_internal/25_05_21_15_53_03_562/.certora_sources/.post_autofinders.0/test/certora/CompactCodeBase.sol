...
contract CompactCodeBase {
    ...
    function uniSqrt(uint256 y) external pure returns (uint256 z) {
        if (y > 3) {
            ...
            uint256 x = y / 2 + 1;
            while (x < z) {
                z = x;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000001,z)}
                x = (y / x + x) / 2;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000002,x)}
            }
        } else if (y != 0) {
            z = 1;
        }
    ...
    function mathMastersSqrtStripped(
        uint256 x
